...
description: How @mongez/vite reads an env variable during vite build and writes it into config.base so assets are referenced from the correct CDN or subpath URL.
when_to_use: User needs to set a CDN or production base URL for Vite assets, user is configuring envBaseUrlKey or the PUBLIC_URL env variable, user wants config.base to be derived from an env file during vite build, user asks why assets load from the wrong origin in production.
---
