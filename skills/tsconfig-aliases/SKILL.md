...
description: How @mongez/vite mirrors compilerOptions.paths from tsconfig.json into Vite's resolve.alias so TypeScript path aliases work in both tsc and the Vite dev server without a separate plugin.
when_to_use: User has TypeScript path aliases in tsconfig.json that don't resolve in Vite, user is configuring linkTsconfigPaths or tsconfigAlias options, user wants to understand how @/* or custom path mappings are synced between TS and Vite.
---
